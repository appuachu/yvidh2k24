          ...
          {dept.map((image) => (
            <section className='eventdetailsmaincontainer'  id={image.id} key={image.id} style={{ display: activeId === image.id ? 'flex' : 'none' }}>
                  ...
                  <div className='eventdetails'>
                    ...
                    <h2> </h2>
                    { image.filelink && (
                      <div style={{ display:'flex',flexDirection:'row',marginRight:'10px'}}><p className='eventimageptag'> </p><DownloadPDF pdflink={image.filelink} pdfname={image.filename}/></div>
                    )}

                    {image.reglink && (
                  <div className='register-button'>
                    <a href={image.reglink} className='register-link'>
                      Register Now
                    </a>
                  </div>
                )}
